...
class App extends Component {
  state = {
    ...
    cuurentImagId: imagesList[0].id,
    gameOnOff: true,
  }

  onChangeGame = () => {
    this.setState({gameOnOff: true, second: 60, score: 0})
  }
  ...
  onThumbclick = id => {
    const {cuurentImagId} = this.state
    ...
    if (cuurentImagId === id) {
      ...
      this.setState({currentImageUrl: imagesList[randomNumber].imageUrl})
      this.setState({cuurentImagId: imagesList[randomNumber].id})
    } else {
      this.setState({gameOnOff: false})
      clearInterval(this.timerId)
    }
  ...
  render() {
    const {score, second, category, currentImageUrl, gameOnOff} = this.state
      ...
      <div>
        ...
        {gameOnOff ? (
          <div className="container-06">
            <img src={currentImageUrl} className="main-image" alt="match" />
            <ul className="tab-main-list">
              {tabsList.map(each => (
                <TabItem
                  TabDetails={each}
                  key={each.tabId}
                  onTabIdChange={this.onTabIdChange}
                />
              ))}
            </ul>
            <ul className="main-list-content">
              {filterList.map(each => (
                <ListItemContent
                  details={each}
                  key={each.id}
